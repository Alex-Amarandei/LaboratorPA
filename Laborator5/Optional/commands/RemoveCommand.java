...
public class RemoveCommand extends Command {
    public RemoveCommand(String commandLine) {
        super(commandLine);
    }
    ...
    public Catalog execute(Catalog catalog) throws CatalogException {
        if (arguments.size() < 2) throw new
                InvalidNumberOfArgumentsException("Too few arguments for Remove Command, the expected number is 2.");

        if (arguments.size() > 2) throw new
                InvalidNumberOfArgumentsException("Too many arguments for Remove Command, the expected number is 2.");
        ...
        catalog.remove(catalog.findByName(arguments.get(1)));

        System.out.println("Removed " + arguments.get(1));
        return catalog;
    }
}
